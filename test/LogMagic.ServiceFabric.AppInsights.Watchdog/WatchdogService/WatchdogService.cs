...
namespace WatchdogService
{
   ...
   internal sealed class WatchdogService : StatelessService
   {
      public WatchdogService(StatelessServiceContext context)
          : base(context)
      {
         IServiceConfig config = new ConfigurationBuilder<IServiceConfig>()
            .UseEnvironmentVariables()
            .Build();

         L.Config
            .EnrichWith.AzureServiceFabricContext(context)
            .EnrichWith.Constant("IsWatchdog", "true")
            .WriteTo.AzureApplicationInsights(config.AppInsightsKey,
               new WriterOptions
               {
                   CollectPerformanceCounters = true,
                   FlushOnWrite = false,
                   EnableQuickPulse = false
               })
            .WriteTo.Trace();
      }
